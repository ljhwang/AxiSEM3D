...
StationOutput::buildInparam(int gindex, double dt, double tmin_simu, double tmax_simu) {
  ...
  bool sourceCentered = gm.getWithLimits<bool>(rootl + ":horizontal_x1_x2",
      {{"LATITUDE_LONGITUDE", false}, {"DISTANCE_AZIMUTH", true}, {"XY_CARTESIAN", true}});
  bool xy = gm.getWithLimits<bool>(rootl + ":horizontal_x1_x2",
      {{"LATITUDE_LONGITUDE", false}, {"DISTANCE_AZIMUTH", false}, {"XY_CARTESIAN", true}});
  bool useDepth =
      gm.getWithLimits<bool>(rootl + ":vertical_x3", {{"RADIUS", false}, {"DEPTH", true}});
  ...
  return std::make_shared<const StationOutput>(groupName,
      fileName,
      sourceCentered,
      xy,
      ellipticity,
...
// verbose
std::string
StationOutput::verbose(double dt, int numRecordSteps, int numStations) const {
  // title
  std::stringstream ss;
  ...
  ss << boxEquals(4, width, "radial geometry", mUndulatedGeometry ? "undulated" : "reference");
...
void
StationOutput::release(const SE_Model& sem,
    Domain& domain,
    double dt,
    double tmin_simu,
    double tmax_simu,
    int nTotalSteps) {
  // group count
  int groupCount = inparam::gInparamOutput.get<int>("list_of_station_groups:[?]");
  ...
  for (int gindex = 0; gindex < groupCount; gindex++) {
    ...
    std::shared_ptr<const StationOutput> stgrp = buildInparam(gindex, dt, tmin_simu, tmax_simu);
      ...
      for (const std::string& line : lines) {
        ...
        if (words.size() != 5 && words.size() != 6) {
          throw std::runtime_error("StationOutput::release || "
                                   ...
                                   " || Ascii file: " +
              io::popInputDir(stgrp->mFileName) + " || Station group name: " + stgrp->mGroupName);
        }
        // key
        const std::string& stKey = words[1] + "." + words[0];
        if (keyCrds.find(stKey) != keyCrds.end()) {
          throw std::runtime_error("StationOutput::release || Station key must be unique. || "
                                   "Duplicated station key: " +
              stKey +
              " || "
              "Station group name: " +
              stgrp->mGroupName);
    ...
    for (int ist = 0; ist < numStations; ist++) {
      ...
      // locate in mesh
      int quadTag = sem.locateInplane(spz({0, 2}).transpose(), stgrp->mFluid);
      if (quadTag != -1) {
    ...
    if (stationRank.size() != numStations) {
      for (int ist = 0; ist < numStations; ist++) {
        if (stationRank.find(ist) == stationRank.end()) {
          throw std::runtime_error("StationOutput::release || Error locating station in mesh."
                                   " || Station key: " +
              stKeys[ist] + " || Station group name: " + stgrp->mGroupName);
        }
      }
    }
    ...
    if (stgrp->mFluid) {
      SGF = std::make_unique<StationGroup<StationFluid>>(stgrp->mGroupName,
          nRecSteps,
          sampleIntv,
          stgrp->mTmin - dt / 2.,
          stgrp->mTmax + dt / 2.,
      ...
      SGS = std::make_unique<StationGroup<StationSolid>>(stgrp->mGroupName,
          nRecSteps,
          sampleIntv,
          stgrp->mTmin - dt / 2.,
          stgrp->mTmax + dt / 2.,
    ...
    for (int ist = 0; ist < numStations; ist++) {
      ...
      if (stgrp->mFluid) {
        ...
        st->setElement(quads[stationQuad[ist]].getFluidElement(), inplaneFactor);
        SGF->addStation(st);
      } else {
        std::unique_ptr<StationSolid> st =
            std::make_unique<StationSolid>(stKeys[ist], spz(1), theta, baz);
        st->setElement(quads[stationQuad[ist]].getSolidElement(), inplaneFactor);
        SGS->addStation(st);
      }
    }
