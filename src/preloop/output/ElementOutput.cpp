...
ElementOutput::buildInparam(int gindex, double dt, double tmin_simu, double tmax_simu) {
  // group name and key in inparam
  const InparamYAML& gm = inparam::gInparamOutput;
  std::string root = "list_of_element_groups";
  const std::string& groupName = gm.get<std::string>(root + ":{" + toString(gindex) + "}");
  ...
  int edgeDim = gm.getWithLimits<int>(
      rooti + ":edge_dimension", {{"HORIZONTAL", 0}, {"VERTICAL", 1}, {"BOTH", -1}});
  double edgeCoord = 0.;
  if (edgeDim >= 0) {
  ...
  if (ipols.front() < 0 || ipols.back() > spectral::nPol) {
    throw std::runtime_error("ElementOutput::buildInparam || elements:GLL_points_one_edge must be "
                             "ranged between 0 and " +
        bstring::toString(spectral::nPol) + ".");
  }
  if (ipols.size() == 0) {
    throw std::runtime_error("ElementOutput::buildInparam || elements:GLL_points_one_edge cannot "
                             "be empty.");
  }
  ...
  for (int iphi = 0; iphi < nphi; iphi++) {
    double phi =
        gm.getWithBounds(roota + ":phi_list:[" + toString(iphi) + "]", 0., 2. * numerical::dPi);
    phis.push_back(phi);
  }
  // lat lon
  int nll = gm.get<int>(roota + ":lat_lon_list:[?]");
  for (int ill = 0; ill < nll; ill++) {
    const std::string& key = roota + ":lat_lon_list:[" + toString(ill) + "]";
  ...
  channel::WavefieldCS wcs = gm.getWithLimits<channel::WavefieldCS>(rootw + ":coordinate_frame",
      {{"spz", channel::WavefieldCS::spz}, {"RTZ", channel::WavefieldCS::RTZ}});
  bool fluid = gm.getWithLimits<bool>(rootw + ":medium", {{"SOLID", false}, {"FLUID", true}});
  const std::vector<std::string> userChannels = gm.getVector<std::string>(rootw + ":channels");
  ...
  return std::make_shared<const ElementOutput>(groupName,
      minR,
      maxR,
      minZ,
      maxZ,
...
// verbose
std::string
ElementOutput::verbose(double dt, int numRecordSteps, int npnts, int nphis, int numElements) const {
  // title
  std::stringstream ss;
  ...
  if (nphis == 0) {
    ...
    for (int iphi = 0; iphi < mPhis.size(); iphi++) {
      ss << boxEquals(6, width - 2, toString(iphi + 1) + ") azimuth", mPhis[iphi]);
    }
    for (int ilat = 0; ilat < mLats.size(); ilat++) {
      const std::string& key = toString(mPhis.size() + ilat + 1) + ") latitude, longitude";
      const std::string& val = toString(mLats[ilat]) + ", " + toString(mLons[ilat]);
  ...
  ss << boxEquals(4, width, "output coordinate system", channel::WavefieldCS_Str.at(mWCS));
  ...
  ss << boxEquals(4, width, "user-specified channels", mUserChannels, "=", true);
...
void
ElementOutput::release(const SE_Model& sem,
    Domain& domain,
    double dt,
    double tmin_simu,
    double tmax_simu,
    ...
    double distTol) {
  // group count
  int groupCount = inparam::gInparamOutput.get<int>("list_of_element_groups:[?]");
  ...
  for (int gindex = 0; gindex < groupCount; gindex++) {
    ...
    std::shared_ptr<const ElementOutput> elgrp = buildInparam(gindex, dt, tmin_simu, tmax_simu);
    ...
    if (elgrp->mEdgeDim < 0) {
      ...
      for (int qTag : opQuads) {
        ...
        for (int iedge = 0; iedge < 4; iedge++) {
          ...
          if (xmin < x(iedge) && x(iedge) < xmax && xmin < x(jedge) && x(jedge) < xmax) {
            edge = iedge;
            break;
          }
        }
    ...
    int npnts = (elgrp->mEdgeDim < 0 ? (int)elgrp->mIPols.size() * (int)elgrp->mIPols.size()
                                     : (int)elgrp->mIPols.size());
    if (elgrp->mFluid) {
      EGF = std::make_unique<ElementOpGroup<ElementOpFluid>>(elgrp->mGroupName,
          nRecSteps,
          sampleIntv,
          elgrp->mTmin - dt / 2.,
          elgrp->mTmax + dt / 2.,
      ...
      EGS = std::make_unique<ElementOpGroup<ElementOpSolid>>(elgrp->mGroupName,
          nRecSteps,
          sampleIntv,
          elgrp->mTmin - dt / 2.,
          elgrp->mTmax + dt / 2.,
    ...
    for (int ielem = 0; ielem < opQuadsUse.size(); ielem++) {
      const std::vector<int>& ipnts = (elgrp->mEdgeDim < 0) ? ipntsElemUniform : ipntsElem[ielem];
      if (elgrp->mFluid) {
        std::unique_ptr<ElementOpFluid> eop = std::make_unique<ElementOpFluid>(ipnts);
        eop->setElement(quads[opQuadsUse[ielem]].getFluidElement());
        EGF->addElementOp(eop);
      } else {
        std::unique_ptr<ElementOpSolid> eop = std::make_unique<ElementOpSolid>(ipnts);
    ...
    if (io::gVerbose != io::VerboseLevel::None && mpi::root()) {
      ...
      ss << elgrp->verbose(dt, nRecSteps, npnts, (int)phisToUse.size(), nelemGlobal);
