...
Material::Material(const ExodusMesh& exodusMesh, const eigen::DMat24& nodalSZ, bool axial) {
  ...
  vector_tools::linearInterpSorted(radialCoords, r.mean(), index0, index1, factor0, factor1);
...
Material::addProperty3D(const std::string& propKey,
  ...
  // ANISO
  if (currentRheology() == RheologyType::ANISO && propKey.front() == 'V') {
  ...
  if (currentRheology() == RheologyType::ISO &&
      (propKey == "VPV" || propKey == "VPH" || propKey == "VSV" || propKey == "VSH" ||
          propKey == "ETA")) {
    evolveISO_TISO();
  }
  // ISO -> TISO -> ANISO
...
Material::finished3D() {
  ...
  if (currentRheology() == RheologyType::TISO) {
    ...
    if ((vpv - vph).norm() < vpv.norm() * numerical::dEpsilon &&
        (vsv - vsh).norm() < vsv.norm() * numerical::dEpsilon &&
        (eta.array() - 1.).matrix().norm() < eta.norm() * numerical::dEpsilon) {
      // add ISO
...
Material::getPointwiseRhoVpVs(
  ...
  if (currentRheology() == RheologyType::FLUID) {
    ...
    const NodalPhysicalProperty& kp = (C11 + C22 + C33 + (C12 + C23 + C13) * 2.) / 9.;
    ...
    // kappa, mu in Voigt average
    const NodalPhysicalProperty& kp = (A * 4. + C + F * 4. - N * 4.) / 9.;
    const NodalPhysicalProperty& mu = (A + C - F * 2. + L * 6. + N * 5.) / 15.;
    // vp, vs
    vp = ((kp + mu * (4. / 3.)) / rh).pow(.5).getPointwise();
    vs = (mu / rh).pow(.5).getPointwise();
  } else {
...
Material::createAnisotropic(
  ...
  // elastic
  if (elastic1D) {
    return std::make_unique<Anisotropic>(attenuation,
        toPP(C11),
        toPP(C12),
        toPP(C13),
        toPP(C14),
...
Material::createTISO(
  ...
  if (elastic1D) {
    ...
    return std::make_unique<TransverselyIsotropic>(attenuation, A, C, F, L, N);
  }
}
...
Material::createIsotropic(
  ...
  // elastic
  if (elastic1D) {
    return std::make_unique<Isotropic>(attenuation, toPP(lambda), toPP(mu));
  } else {
    return std::make_unique<Isotropic>(attenuation, lambda, mu);
  }
}
