...
// step 1: constructor
SE_Model::SE_Model(const ExodusMesh& exodusMesh,
    const ABC& abc,
  ...
  for (const std::shared_ptr<const Model3D>& m : models3D) {
    std::shared_ptr<const Geometric3D> g3D = std::dynamic_pointer_cast<const Geometric3D>(m);
    if (g3D) {
      timer::gPreloopTimer.message("Adding geometric3D model: " + g3D->getModelName());
...
void
SE_Model::release(const ABC& abc,
    const LocalMesh& localMesh,
  ...
  for (int icom = 0; icom < localMesh.mCommProc.size(); icom++) {
    ...
    for (int igll : localMesh.mCommMyGLL[icom]) {
      meshPoints.push_back({mGLLPoints[igll].getGlobalTag(),
          mGLLPoints[igll].getSolidPoint(),
          mGLLPoints[igll].getFluidPoint()});
    }
    std::unique_ptr<MessageRank> msgRank = std::make_unique<MessageRank>(rankOther, meshPoints);
...
SE_Model::measureCost(const ExodusMesh& exodusMesh,
  ...
  for (int iquad = 0; iquad < mQuads.size(); iquad++) {
    ...
    for (int jquad = iquad + 1; jquad < mQuads.size(); jquad++) {
      ...
      if (elemSign(jquad) == esign) {
        ...
        // use minimum
        elemCostLibrary.at(esign) = std::min(jmeasured, elemCostLibrary.at(esign));
  ...
  for (int igll = 0; igll < mGLLPoints.size(); igll++) {
    ...
    for (int jgll = igll + 1; jgll < mGLLPoints.size(); jgll++) {
      ...
      if (pointSign(jgll) == psign) {
        ...
        // use minimum
        pointCostLibrary.at(psign) = std::min(jmeasured, pointCostLibrary.at(psign));
        if (++sameKindFound == numSameKind) {
          break;
        }
      }
  ...
  if (mpi::root() && timer::gPreloopTimer.isActivated()) {
    const std::string& fname = (io::gOutputDirectory + "/develop/measured_element_costs.log");
    std::ofstream ofs(fname);
    if (!ofs) {
      throw std::runtime_error("SE_Model::measureCost || Error opening "
                               "or creating log file: || " +
          fname);
    }
    ofs << "Elements:\n";
  ...
  for (int iquad = 0; iquad < mQuads.size(); iquad++) {
    ...
    if (measurePoint) {
      for (int ipnt = 0; ipnt < spectral::nPEM; ipnt++) {
        ...
        weightsLocal(iquad) +=
            (pointCostLibrary.at(pointSign(igll)) / mGLLPoints[igll].getElementCount());
      }
    }
  }
...
// verbose
std::string
SE_Model::verbose(const std::string& titleSEM) const {
...
void
SE_Model::initScanningOnPoints(bool vertexOnly) const {
  if (vertexOnly) {
