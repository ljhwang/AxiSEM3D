...
// initialize
void
StationIO_Ascii::initialize(const std::string& groupName,
    int numRecordSteps,
  ...
  if (mStationCentric) {
    ...
    for (int ist = 0; ist < nst; ist++) {
      ...
      (*mFileStreams[ist + tfile])
          << bufferFields.slice(loc, len).reshape(shape).shuffle(shuffle) << "\n";
      if (mFlush) {
...
// create file
void
StationIO_Ascii::createFileStream(const std::string& varKey, int precision) {
