...
Station::rotate(eigen::RMatX3_RM& V, int nrow, int k, double angle) {
  ...
  V.block(0, i, nrow, 1) = (sColBuffer.topRows(nrow) * cosA + V.block(0, j, nrow, 1) * sinA);
  V.block(0, j, nrow, 1) = (-sColBuffer.topRows(nrow) * sinA + V.block(0, j, nrow, 1) * cosA);
}
...
Station::rotate(eigen::RMatX9_RM& T, int nrow, int k, double angle) {
  ...
  for (int n = 0; n < 3; n++) {
    ...
    T.block(0, I, nrow, 1) = (sColBuffer.topRows(nrow) * cosA + T.block(0, J, nrow, 1) * sinA);
    T.block(0, J, nrow, 1) = (-sColBuffer.topRows(nrow) * sinA + T.block(0, J, nrow, 1) * cosA);
  }
  // Tmn = Smp Qnp
  for (int p = 0; p < 3; p++) {
    int I = p * 3 + i;
