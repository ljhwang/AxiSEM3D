...
template <typename floatT> class GradientQuadrature {
  /////////////////////////// typedef ///////////////////////////
  ...
  GradientQuadrature(const GradientQuadrature& other) :
      mDsDxii(other.mDsDxii), mDsDeta(other.mDsDeta), mDzDxii(other.mDzDxii),
      mDzDeta(other.mDzDeta), m1overS(other.m1overS), mAxial(other.mAxial),
  ...
  GradientQuadrature(const GradientQuadrature& other) :
      mDsDxii(other.mDsDxii), mDsDeta(other.mDsDeta), mDzDxii(other.mDzDxii),
      mDzDeta(other.mDzDeta), m1overS(other.m1overS), mAxial(other.mAxial),
      mDsDxii_IF(other.mDsDxii_IF), mDsDeta_IF(other.mDsDeta_IF), mDzDxii_IF(other.mDzDxii_IF),
      mDzDeta_IF(other.mDzDeta_IF), m1overS_IF(other.m1overS_IF) {
    // nothing
  }
#endif
  ...
  // grad 1 -> 3
  void
  computeGrad3(const vec_ar1_CMatPP_RM& p, vec_ar3_CMatPP_RM& p_i, int nu_1) const {
    ////// higher-order terms //////
  ...
  // 20->6 21->7 22->8
  void
  computeGrad9(const vec_ar3_CMatPP_RM& ui, vec_ar9_CMatPP_RM& ui_j, int nu_1) const {
    ////// higher-order terms //////
  ...
  // 20->4 21->3 22->2
  void
  computeGrad6(const vec_ar3_CMatPP_RM& ui, vec_ar6_CMatPP_RM& eij, int nu_1) const {
    ////// higher-order terms //////
  ...
  computeQuad3(const vec_ar3_CMatPP_RM& q_i, vec_ar1_CMatPP_RM& q, int nu_1) const {
...
#endif
    ////// higher-order terms //////
    computeQuad_dSdZ(nu_1, q_i, 0, 2, q, 0, xDsDxii_IF, xDsDeta_IF, xDzDxii_IF, xDzDeta_IF);
    ...
    computeQuad_1overS_addTo(nu_1, sA, 0, q, 0, x1overS_IF, xDzDxii_IF, xDzDeta_IF);
  }
  ...
  computeQuad9(const vec_ar9_CMatPP_RM& fi_j, vec_ar3_CMatPP_RM& fi, int nu_1) const {
...
#endif
    ////// higher-order terms //////
    computeQuad_dSdZ(nu_1, fi_j, 0, 2, fi, 0, xDsDxii_IF, xDsDeta_IF, xDzDxii_IF, xDzDeta_IF);
    computeQuad_dSdZ(nu_1, fi_j, 3, 5, fi, 1, xDsDxii_IF, xDsDeta_IF, xDzDxii_IF, xDzDeta_IF);
    computeQuad_dSdZ(nu_1, fi_j, 6, 8, fi, 2, xDsDxii_IF, xDsDeta_IF, xDzDxii_IF, xDzDeta_IF);
    ...
    computeQuad_1overS_addTo(nu_1, sA, 0, fi, 0, x1overS_IF, xDzDxii_IF, xDzDeta_IF);
    computeQuad_1overS_addTo(nu_1, sA, 1, fi, 1, x1overS_IF, xDzDxii_IF, xDzDeta_IF);
    computeQuad_1overS_addTo(nu_1, sA, 2, fi, 2, x1overS_IF, xDzDxii_IF, xDzDeta_IF);
  }
...
#endif
    ////// higher-order terms //////
    computeQuad_dSdZ(nu_1, sij, 0, 4, fi, 0, xDsDxii_IF, xDsDeta_IF, xDzDxii_IF, xDzDeta_IF);
    computeQuad_dSdZ(nu_1, sij, 5, 3, fi, 1, xDsDxii_IF, xDsDeta_IF, xDzDxii_IF, xDzDeta_IF);
    computeQuad_dSdZ(nu_1, sij, 4, 2, fi, 2, xDsDxii_IF, xDsDeta_IF, xDzDxii_IF, xDzDeta_IF);
  ...
  // 20->6 21->7 22->8
  void
  computeQuad9_NoIntegration(const vec_ar9_CMatPP_RM& fi_j, vec_ar3_CMatPP_RM& fi, int nu_1) const {
    ////// higher-order terms //////
    computeQuad_dSdZ(nu_1, fi_j, 0, 2, fi, 0, mDsDxii, mDsDeta, mDzDxii, mDzDeta);
    computeQuad_dSdZ(nu_1, fi_j, 3, 5, fi, 1, mDsDxii, mDsDeta, mDzDxii, mDzDeta);
    computeQuad_dSdZ(nu_1, fi_j, 6, 8, fi, 2, mDsDxii, mDsDeta, mDzDxii, mDzDeta);
    ...
    computeQuad_1overS_addTo(nu_1, sA, 0, fi, 0, m1overS, mDzDxii, mDzDeta);
    computeQuad_1overS_addTo(nu_1, sA, 1, fi, 1, m1overS, mDzDxii, mDzDeta);
    computeQuad_1overS_addTo(nu_1, sA, 2, fi, 2, m1overS, mDzDxii, mDzDeta);
  }
  ...
  // 20->4 21->3 22->2
  void
  computeQuad6_NoIntegration(const vec_ar6_CMatPP_RM& sij, vec_ar3_CMatPP_RM& fi, int nu_1) const {
    ////// higher-order terms //////
    computeQuad_dSdZ(nu_1, sij, 0, 4, fi, 0, mDsDxii, mDsDeta, mDzDxii, mDzDeta);
    computeQuad_dSdZ(nu_1, sij, 5, 3, fi, 1, mDsDxii, mDsDeta, mDzDxii, mDzDeta);
    computeQuad_dSdZ(nu_1, sij, 4, 2, fi, 2, mDsDxii, mDsDeta, mDzDxii, mDzDeta);
  ...
  void
  computeGrad_dSdZ(
      int nu_1, const VecArPP_F& F, int dimF, VecArPP_dF& dF, int dim_dFdS, int dim_dFdZ) const {
    // GT_xii
    ...
    for (int alpha = 0; alpha < nu_1; alpha++) {
      ...
      dF[alpha][dim_dFdS] = (sX.cwiseProduct(mDzDeta) + sY.cwiseProduct(mDzDxii));
      dF[alpha][dim_dFdZ] = (sX.cwiseProduct(mDsDeta) + sY.cwiseProduct(mDsDxii));
    }
  }
  ...
  void
  computeGrad_1overS_setTo(
      int nu_1, const VecArPP_A& A, int dimA, VecArPP_AoS& AoS, int dimAoS) const {
    // GT_xii
    const RMatPP_RM& GT_xii = mAxial ? sGT_GLJ : sGT_GLL;
    // loop over alpha
    for (int alpha = 0; alpha < nu_1; alpha++) {
      ...
      if (mAxial) {
        ...
        AoS[alpha][dimAoS].row(0) =
            (sX.row(0).cwiseProduct(mDzDeta.row(0)) + sY.row(0).cwiseProduct(mDzDxii.row(0)));
      }
    }
  }
  ...
  void
  computeQuad_dSdZ(int nu_1,
      const VecArPP_dF& dF,
      int dim_dFdS,
      int dim_dFdZ,
      VecArPP_F& F,
      int dimF,
      const RMatPP_RM& dsdxii,
      const RMatPP_RM& dsdeta,
    ...
    // loop over alpha
    for (int alpha = 0; alpha < nu_1; alpha++) {
      sX = (dF[alpha][dim_dFdS].cwiseProduct(dzdeta) + dF[alpha][dim_dFdZ].cwiseProduct(dsdeta));
      sY = (dF[alpha][dim_dFdS].cwiseProduct(dzdxii) + dF[alpha][dim_dFdZ].cwiseProduct(dsdxii));
  ...
  template <class VecArPP_A, class VecArPP_AoS>
  void
  computeQuad_1overS_addTo(int nu_1,
      const VecArPP_A& A,
      int dimA,
      VecArPP_AoS& AoS,
      int dimAoS,
