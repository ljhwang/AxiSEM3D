...
class AttenuationFull : public Attenuation {
...
#endif
    for (int alpha = 0; alpha < nu_1; alpha++) {
      apply<CaseFA::_1D_FR>(strain, stress, alpha, mDStress_FR, mMemVar_FR, mDLambda, mDMu, xDMu2);
    }
  }
...
#endif
    if (m1D) {
      apply<CaseFA::_1D_CD>(strain, stress, nr, mDStress_CD, mMemVar_CD, mDLambda, mDMu, xDMu2);
    } else {
      apply<CaseFA::_3D_CD>(strain, stress, nr, mDStress_CD, mMemVar_CD, mDLambda, mDMu, xDMu2);
    }
  }
  ...
  subtractMemFromStress(const std::vector<FMat6>& memVar, FMat6& stress, int nx) {
  ...
  template <CaseFA CASE, class FMat6>
  static void
  apply(const FMat6& strain,
      FMat6& stress,
      int nx,
      FMat6& dStress,
      std::vector<FMat6>& memVar,
    ...
    Isotropic::strainToStress<CASE>(strain, dStress, nx, dLambda, dMu, dMu2);
