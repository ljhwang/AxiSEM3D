...
class AttenuationCG4 : public Attenuation {
  ...
  checkCompatibility(int nr, bool elemInFourier, bool elastic1D) {
    ...
    // parameters
    mDMu.checkCompatibility(m1D, nr, elemInFourier, "mDMu", "AttenuationCG4");
  ...
  apply(const eigen::vec_ar6_CMatPP_RM& strain, eigen::vec_ar6_CMatPP_RM& stress, int nu_1) {
    ...
    for (int alpha = 0; alpha < nu_1; alpha++) {
      apply<CaseFA::_1D_FR>(
          strain, stress, alpha, mDStress_FR, mMemVar_FR, sStrain4_FR, mDLambda, mDMu, xDMu2);
    }
  }
  ...
  apply(const eigen::RMatXN6& strain, eigen::RMatXN6& stress, int nr) {
    ...
    if (m1D) {
      apply<CaseFA::_1D_CD>(
          strain, stress, nr, mDStress_CD, mMemVar_CD, sStrain4_CD, mDLambda, mDMu, xDMu2);
    } else {
      apply<CaseFA::_3D_CD>(
          strain, stress, nr, mDStress_CD, mMemVar_CD, sStrain4_CD, mDLambda, mDMu, xDMu2);
    }
  ...
  /////////////////// strain4 => dStress ///////////////////
  template <CaseFA CASE, class FCG6>
  static void
  strainToStressCG4(const FCG6& strain4,
      FCG6& dStress,
      int nx,
  ...
  apply(const FMat6& strain,
    ...
    strainToStressCG4<CASE>(strain4, dStress, nx, dLambda, dMu, dMu2);
