...
class Attenuation {
  ...
  virtual std::unique_ptr<Attenuation>
  clone() const = 0;
  ...
  updateMemAlphaBeta(const FMat6& dStress, std::vector<FMat6>& memVar, int nx) {
    for (int isls = 0; isls < nsls(); isls++) {
      for (int idim = 0; idim < 6; idim++) {
        memVar[isls][nx][idim] =
            sAlpha[isls] * memVar[isls][nx][idim] + sBetta[isls] * dStress[nx][idim];
      }
    }
  }
  // CASE != _1D_FR
  ...
  updateMemAlphaBeta(const FMat6& dStress, std::vector<FMat6>& memVar, int nx) {
    for (int isls = 0; isls < nsls(); isls++) {
      memVar[isls] = (sAlpha[isls] * memVar[isls] + sBetta[isls] * dStress);
    }
  }
