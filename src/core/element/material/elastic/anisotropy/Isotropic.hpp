...
class Isotropic : public Elastic {
  ...
  strainToStress_FR(
    ...
    for (int alpha = 0; alpha < nu_1; alpha++) {
      strainToStress<CaseFA::_1D_FR>(strain, stress, alpha, mLambda, mMu, xMu2);
    }
...
#endif
    if (m1D) {
      strainToStress<CaseFA::_1D_CD>(strain, stress, nr, mLambda, mMu, xMu2);
    } else {
      strainToStress<CaseFA::_3D_CD>(strain, stress, nr, mLambda, mMu, xMu2);
    }
