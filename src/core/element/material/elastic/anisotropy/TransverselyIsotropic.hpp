...
class TransverselyIsotropic : public Elastic {
  ...
  strainToStress_FR(
    ...
    for (int alpha = 0; alpha < nu_1; alpha++) {
      strainToStress<CaseFA::_1D_FR>(strain, stress, alpha, mA, mC, mF, mL, mN, xMinusN2);
    }
...
#endif
    if (m1D) {
      strainToStress<CaseFA::_1D_CD>(strain, stress, nr, mA, mC, mF, mL, mN, xMinusN2);
    } else {
      strainToStress<CaseFA::_3D_CD>(strain, stress, nr, mA, mC, mF, mL, mN, xMinusN2);
    }
