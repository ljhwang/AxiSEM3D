...
void
WavefieldInjection::initializeElements(
    std::vector<int>& interiorQuadTags, std::vector<int>& exteriorQuadTags, const Domain& domain) {
  ...
  for (const auto& elem : domain.getSolidElements()) {
    // interior
    if (vector_tools::findSortedUnique(interiorQuadTags, elem->getQuadTag())) {
      inSolidElems.push_back(elem);
      continue; // cannot be both exterior and interior
    }
    // exterior
    if (vector_tools::findSortedUnique(exteriorQuadTags, elem->getQuadTag())) {
  ...
  for (const auto& inElem : inSolidElems) {
    ...
    std::vector<int> pointsOnWJB = inElem->findBoundaryPointsByTag(exPointMeshTags);
...
void
WavefieldInjection::initializeElements(std::vector<int>& interiorQuadTags,
  ...
  for (const auto& inElem : inSolidElems) {
    ...
    std::vector<int> pointsOnWJB =
        inElem->findBoundaryPointsByCrds(boundaryCrdsRorZ, boundaryCrdsTorS, distTol);
    // found an interior element
    if (pointsOnWJB.size() != 0) {
      std::shared_ptr<SolidElementInteriorWJ> inElemWJ =
          std::make_shared<SolidElementInteriorWJ>(*inElem, pointsOnWJB);
...
void
WavefieldInjection::initializeSTFs(const std::string& ncFileNameSolid,
  ...
  // no injection
  if (mInteriorSolidElements.size() + mInteriorFluidElements.size() == 0) {
...
// set in domain
void
WavefieldInjection::setInDomain(Domain& domain) const {
...
// apply
void
WavefieldInjection::apply(int timeStep, double time) const {
  // solid
  for (const auto& elem : mInteriorSolidElements) {
...
// info
void
WavefieldInjection::infoSolid(std::vector<std::string>& quadKeys,
  ...
  for (int ielem = 0; ielem < mInteriorSolidElements.size(); ielem++) {
    ...
    mInteriorSolidElements[ielem]->receiverInfo(recKeys[ielem], rec_spz[ielem]);
  }
}

// info
void
WavefieldInjection::infoFluid(std::vector<std::string>& quadKeys,
    std::vector<int>& quadNrs,
    std::vector<std::vector<int>>& quadBoundaryPnts,
    std::vector<std::vector<std::string>>& recKeys,
    std::vector<eigen::DMatXX>& rec_spz) const {
  ...
  for (int ielem = 0; ielem < mInteriorFluidElements.size(); ielem++) {
    ...
    mInteriorFluidElements[ielem]->receiverInfo(recKeys[ielem], rec_spz[ielem]);
  }
}
