...
class NetCDF_Writer {
  ...
  // open
  void
  open(const std::string& fname, bool overwrite);

  // open parallel
  void
  openParallel(const std::string& fname);
  ...
  int
  defineVariable(const std::string& vname,
      const std::vector<std::pair<std::string, numerical::Int>>& dims,
      const scalar& fill,
      bool collective = true) const {
    // define dimensions
    std::vector<int> dimids(dims.size(), -1);
    for (int idim = 0; idim < dims.size(); idim++) {
      // find dimension ID
      if (nc_inq_dimid(mFileID, dims[idim].first.c_str(), &(dimids[idim])) != NC_NOERR) {
        // if not found, create it
        netcdf::error(
            nc_def_dim(mFileID, dims[idim].first.c_str(), (size_t)dims[idim].second, &dimids[idim]),
            "nc_def_dim",
            mFileName);
      }
    }
    ...
    // fill with constant
    netcdf::error(nc_def_var_fill(mPWD, varid, NC_FILL, &fill), "nc_def_var_fill", mFileName);
  ...
  template <class Container>
  void
  writeVariable(const std::string& vname,
      const Container& val,
      const std::vector<numerical::Int>& starts,
      const std::vector<numerical::Int>& counts) const {
    int varid = netcdf::varID_CheckType(mPWD, vname, mFileName, netcdf::typeNC_V(val));
  ...
  template <class Container>
  void
  writeWholeVariable(const std::string& vname, const Container& val) const {
    int varid = netcdf::varID_CheckType(mPWD, vname, mFileName, netcdf::typeNC_V(val));
  ...
  // create group
  void
  createGroup(const std::string& gname) const;

  // go to group
  void
  goToGroup(const std::string& gname);
  ...
  void
  addAttribute(
      const std::string& attname, const scalar& attvalue, const std::string& vname = "") const {
    if (vname == "") {
      // file attribute
      netcdf::error(
          nc_put_att(mFileID, NC_GLOBAL, attname.c_str(), netcdf::typeNC_T<scalar>(), 1, &attvalue),
          "nc_put_att",
          mFileName);
    } else {
      // variable attribute
      ...
      netcdf::error(
          nc_put_att(mPWD, varid, attname.c_str(), netcdf::typeNC_T<scalar>(), 1, &attvalue),
          "nc_put_att",
          mFileName);
    }
  }
...
template <>
void
NetCDF_Writer::addAttribute<std::string>(
    const std::string& attname, const std::string& attvalue, const std::string& vname) const;
