...
// open
void
NetCDF_Reader::open(const std::string& fname) {
  close();
...
// read string
void
NetCDF_Reader::readString(const std::string& vname, std::vector<std::string>& dest) const {
  // access variable
  ...
  // read and split
  netcdf::error(nc_get_var_text(mFileID, varid, cstr.data()), "nc_get_var_text", mFileName);
