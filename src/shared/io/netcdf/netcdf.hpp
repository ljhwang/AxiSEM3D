...
namespace netcdf {
  ...
  template <typename T>
  inline nc_type
  typeNC_V(const std::vector<T>& val) {
    return typeNC_T<T>();
  }

  // string
  inline nc_type
  typeNC_V(const std::string& val) {
  ...
  inline typename std::enable_if<Eigen::DenseBase<Derived>::IsRowMajor ||
          Eigen::DenseBase<Derived>::ColsAtCompileTime == 1,
      nc_type>::type
  ...
  inline nc_type
  typeNC_V(const Eigen::Tensor<Scalar, R, Eigen::RowMajor>& val) {
  ...
  inline nc_type
  typeNC_V(const Eigen::Tensor<Scalar, 1, Eigen::ColMajor>& val) {
    return typeNC_T<Scalar>();
  }
  ...
  // error handler
  inline void
  error(int retval, const std::string& funcName, const std::string& fname) {
    if (retval != NC_NOERR) {
  ...
  // get id
  inline int
  varID(int fid, const std::string& vname, const std::string& fname) {
    int varid = -1;
  ...
  inline int
  varID_CheckType(
      int fid, const std::string& vname, const std::string& fname, const nc_type& requiredTypeNC) {
    // get id
    int varid = varID(fid, vname, fname);

    // get nc datatype
    nc_type typeInFile;
    error(nc_inq_vartype(fid, varid, &typeInFile), "nc_inq_vartype", fname);
