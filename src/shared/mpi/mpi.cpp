...
namespace mpi {
  ...
  setupGroup(int nprocPerGroup) {
    ...
    for (auto it = nodeID_NP.begin(); it != nodeID_NP.end(); ++it) {
      ...
      internal::iMinNumProcOnNode = std::min(internal::iMinNumProcOnNode, it->second);
      internal::iMaxNumProcOnNode = std::max(internal::iMaxNumProcOnNode, it->second);
    }
    internal::iNumProcPerGroup = std::min(internal::iMaxNumProcOnNode, nprocPerGroup);

    // total number of groups
    internal::iNumGroups = curGroupIndex;

    // my group index
    int myGroupIndex = (myIndexInID / internal::iNumProcPerGroup + nodeID_StartG.at(nodeID));
    ...
    MPI_Comm_split(MPI_COMM_WORLD, myGroupIndex, rank(), &internal::iCommInfer);
#endif
  }
  ...
  void
  bcast(std::vector<std::string>& vecStr, int src) {
    // string to std::vector<char>
    std::vector<std::vector<char>> vvchar;
    vvchar.reserve(vecStr.size());
    if (rank() == src) {
      for (int istr = 0; istr < vecStr.size(); istr++) {
        vvchar.push_back(std::vector<char>(vecStr[istr].begin(), vecStr[istr].end()));
      }
    }
    ...
    if (rank() != src) {
      ...
      for (int istr = 0; istr < vvchar.size(); istr++) {
        vecStr[istr] = std::string(vvchar[istr].begin(), vvchar[istr].end());
      }
    }
  }
  ...
  gather(const std::string& str, std::vector<std::string>& vecStr, int dest) {
    ...
    if (dest < 0 || dest == rank()) {
      ...
      for (int iproc = 0; iproc < nproc(); iproc++) {
        vecStr[iproc] = std::string(vvchar[iproc].begin(), vvchar[iproc].end());
      }
    }
  }
  ...
  verbose() {
    ...
    ss << bstring::boxEquals(0, 24, "min # processors on node", internal::iMinNumProcOnNode);
    ss << bstring::boxEquals(0, 24, "max # processors on node", internal::iMaxNumProcOnNode);
#else
