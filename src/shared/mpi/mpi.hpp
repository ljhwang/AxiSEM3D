...
namespace mpi {
  ...
  namespace internal {
    ...
    template <>
    inline MPI_Datatype
    typeMPI<std::complex<float>>() {
      return MPI_C_FLOAT_COMPLEX;
    }

    template <>
    inline MPI_Datatype
    typeMPI<std::complex<double>>() {
  ...
  template <typename T>
  void
  bcast(std::vector<T>& vec, int src = 0) {
    // size
    int size = 0;
    if (rank() == src) {
      size = (int)vec.size();
  ...
  template <typename T>
  void
  bcast(std::vector<std::vector<T>>& vecVec, int src = 0) {
    ...
    if (rank() == src) {
      ...
      for (int ivec = 0; ivec < vecVec.size(); ivec++) {
        flattened.insert(flattened.end(), vecVec[ivec].begin(), vecVec[ivec].end());
    ...
    if (rank() != src) {
      ...
      for (int ivec = 0; ivec < lengths.size(); ivec++) {
        vecVec[ivec].insert(
            vecVec[ivec].begin(), flattened.begin() + pos, flattened.begin() + pos + lengths[ivec]);
        pos += lengths[ivec];
      }
    }
  }
  ...
  void
  bcastEigen(EigenMat& mat, int src = 0) {
    // size
    int dim[2];
    if (rank() == src) {
      dim[0] = (int)mat.rows();
  ...
  void
  bcast(std::string& str, int src = 0);

  // specialization for vector of string
  void
  bcast(std::vector<std::string>& vecStr, int src = 0);
  ...
  template <typename EigenMat>
  void
  sendVecEigen(int dest, const std::vector<EigenMat>& vecMat, int tag) {
    if (dest == mpi::rank()) {
      return;
    }
    ...
    for (int imat = 0; imat < nmat; imat++) {
      ...
      flattened.insert(
          flattened.end(), vecMat[imat].data(), vecMat[imat].data() + vecMat[imat].size());
    }

    // check: size < max int
    if (flattened.size() > std::numeric_limits<int>::max()) {
      throw std::runtime_error("mpi::sendVecEigen || "
    ...
    // row, col
    MPI_Send(rowCol.data(), nmat * 2, MPI_INT, dest, tag * 3 + 1, internal::iCommCurrent);

    // data
    MPI_Send(flattened.data(),
        sizeInfo[1],
        internal::typeMPI<typename EigenMat::Scalar>(),
        dest,
        tag * 3 + 2,
  ...
  template <typename EigenMat>
  void
  recvVecEigen(int source, std::vector<EigenMat>& vecMat, int tag) {
    if (source == mpi::rank()) {
      return;
    }
    ...
    for (int imat = 0; imat < nmat; imat++) {
      ...
      const EigenMat& mat = Eigen::Map<const EigenMat>(flattened.data() + pos, row, col);
  ...
  template <typename T>
  void
  gather(const std::vector<T>& vec, std::vector<std::vector<T>>& vecVec, int dest) {
    // size
    ...
    if (dest == MPI_ALL || dest == rank()) {
      ...
      for (int iproc = 0; iproc < nproc(); iproc++) {
        std::vector<T> vecRank(vecVecFlatten.begin() + vDisp[iproc],
            vecVecFlatten.begin() + vDisp[iproc] + vSize[iproc]);
  ...
  template <typename EigenMat>
  void
  gatherEigen(const EigenMat& mat, std::vector<EigenMat>& vecMat, int dest) {
    // eigen to vector
    std::vector<typename EigenMat::Scalar> vec(mat.data(), mat.data() + mat.size());
    ...
    if (dest == MPI_ALL || dest == rank()) {
      ...
      for (int iproc = 0; iproc < nproc(); iproc++) {
        vecMat.push_back(Eigen::Map<EigenMat>(vecVec[iproc].data(), vecRow[iproc], vecCol[iproc]));
      }
    }
  }
  ...
  // specialization for string
  void
  gather(const std::string& str, std::vector<std::string>& vecStr, int dest);

  // specialization for vector of string
  void
  gather(const std::vector<std::string>& vecStr,
  ...
  template <typename T>
  void
  scatter(const std::vector<std::vector<T>>& vecVec, std::vector<T>& vec, int src) {
    // to be formed on src
    std::vector<int> vSize;
    std::vector<int> vDisp;
    std::vector<T> vecVecFlatten;
    if (rank() == src) {
      ...
      for (int iproc = 0; iproc < nproc(); iproc++) {
        ...
        vDisp[iproc] = totalSize;
        totalSize += vSize[iproc];
        vecVecFlatten.insert(vecVecFlatten.end(), vecVec[iproc].begin(), vecVec[iproc].end());
      }
    }
  ...
  template <typename EigenMat>
  void
  scatterEigen(const std::vector<EigenMat>& vecMat, EigenMat& mat, int src) {
    // eigen to vector
    std::vector<std::vector<typename EigenMat::Scalar>> vecVec;
    std::vector<int> vecRow, vecCol;
    if (rank() == src) {
      ...
      for (int iproc = 0; iproc < nproc(); iproc++) {
        vecVec.push_back(std::vector<typename EigenMat::Scalar>(
            vecMat[iproc].data(), vecMat[iproc].data() + vecMat[iproc].size()));
  ...
  void
  gather(const std::map<KEY, VAL>& map, std::vector<std::map<KEY, VAL>>& vecMap, int dest) {
  ...
  template <typename KEY, typename VAL>
  void
  aggregate(std::map<KEY, VAL>& map, int dest, MPI_Op op) {
    // gather
    ...
    if (dest == MPI_ALL || dest == rank()) {
      map.clear();
      for (int iproc = 0; iproc < nproc(); iproc++) {
        for (auto it = allMaps[iproc].begin(); it != allMaps[iproc].end(); ++it) {
  ...
  DataMat
  nodalToElemental(const IndexMat& connectivity, const DataMat& nodal, bool collective) {
