...
main(int argc, char* argv[]) {
  ...
  try {
    ...
    std::unique_ptr<const AttBuilder> attBuilder = buildAttenuation(*exodusMesh, dt);
    ...
    releaseToDomain(*sem, *abc, *localMesh, attBuilder, *timeScheme, *domain, "Stage-I");
    ...
    // build SE model
    timer::gPreloopTimer.begin("Spectral element model (ROOT)", '*');
    sem = buildSE_Model(*exodusMesh, *abc, *localMesh, models3D, "Stage-II");
    timer::gPreloopTimer.ended("Spectral element model (ROOT)", '*');

    // domain
    timer::gPreloopTimer.begin("Cost-weighted domain", '*');
    domain = Domain::createEmpty();
    releaseToDomain(*sem, *abc, *localMesh, attBuilder, *timeScheme, *domain, "Stage-II");
    ...
    StationOutput::release(
        *sem, *domain, dt, timeScheme->getT0(), timeScheme->getT1(), timeScheme->getNumTimeSteps());
    timer::gPreloopTimer.ended("Station groups", '*');

    // element output
    timer::gPreloopTimer.begin("Element groups", '*');
    ElementOutput::release(*sem,
        *domain,
        dt,
        timeScheme->getT0(),
        timeScheme->getT1(),
    ...
    setupWavefieldScanning(dt, meshPeriod, timeScheme->getNumTimeSteps(), *sem, *domain);
...
setupEnvironment(int argc, char* argv[]) {
  ...
  // mpi group
  mpi::setupGroup(inparam::gInparamAdvanced.get<int>("mpi:nproc_per_group"));
  ...
  // preloop timer
  timer::setupPreloopDiagnosis(inparam::gInparamAdvanced.get<bool>("develop:diagnose_preloop"));
  if (io::gVerbose == io::VerboseLevel::Detailed) {
...
buildExodusMesh() {
  ...
  std::unique_ptr<ExodusMesh> exodusMesh =
      std::make_unique<ExodusMesh>(inparam::gInparamModel.get<std::string>("model1D:exodus_mesh"));
  if (io::gVerbose != io::VerboseLevel::None) {
...
computeNrFieldAndWeights(ExodusMesh& exodusMesh) {
  ...
  if (mpi::super()) {
    ...
    std::unique_ptr<const NrField> nrField = NrField::buildInparam(exodusMesh);
    if (io::gVerbose != io::VerboseLevel::None) {
    ...
    bool boundByInplane = inparam::gInparamNr.get<bool>("bound_Nr_by_inplane");
    bool useLuckyNumbers = inparam::gInparamAdvanced.get<bool>("develop:fftw_lucky_numbers");
    // compute
  ...
  if (mpi::super()) {
    weights = mpi::nodalToElemental(
        exodusMesh.getConnectivity(), exodusMesh.getNrAtNodes().cast<double>().eval(), true);
    // also differentiate solid (x10) and fluid (x1)
    weights.array() *= (10 - 9 * exodusMesh.getIsElementFluid().array()).cast<double>();
  }
  mpi::enterWorld();
...
buildLocalMesh(const ExodusMesh& exodusMesh,
  ...
  std::unique_ptr<LocalMesh> localMesh = std::make_unique<LocalMesh>(exodusMesh, weights);
  timer::gPreloopTimer.ended("Building local mesh");

  // verbose
  if (io::gVerbose == io::VerboseLevel::Detailed) {
    io::cout << localMesh->verbose("Mesh Partition " + stageKey, weightsKey, weights);
  }

  // plot
  bool plotDD = inparam::gInparamAdvanced.get<bool>("mpi:plot_domain_decomposition");
  if (plotDD) {
...
buildModels3D(const ExodusMesh& exodusMesh,
  ...
  if (oceanload && topABC) {
    io::cout << bstring::warning("main::buildModels3D || "
...
buildSE_Model(const ExodusMesh& exodusMesh,
  ...
  bool useLuckyNumbers = inparam::gInparamAdvanced.get<bool>("develop:fftw_lucky_numbers");
  std::unique_ptr<SE_Model> sem =
      std::make_unique<SE_Model>(exodusMesh, abc, localMesh, models3D, useLuckyNumbers);
  // free dummy memory
...
computeDt(const SE_Model& sem, const ABC& abc) {
  ...
  // enforced
  double dtEnforce =
      inparam::gInparamSource.getWithOptions<double>("time_axis:enforced_dt", {{"NONE", -1.}});
  // finally used
  ...
  if (io::gVerbose != io::VerboseLevel::None) {
    ...
    if (geodesy::isCartesian()) {
      ss << bstring::boxEquals(0, 21, "   location (s,z)", bstring::range(sz(0), sz(1)));
    } else {
      const auto& rt = geodesy::sz2rtheta(sz, false);
      ss << bstring::boxEquals(0, 22, "   location (r,θ)", bstring::range(rt(0), rt(1)));
    }
    if (useMesh) {
  ...
  if (io::gVerboseWarnings && dtEnforce > dtMesh) {
    io::cout << bstring::warning("main::computeDt || "
...
buildTimeScheme() {
  // key
  const std::string& tsKey = inparam::gInparamSource.getWithLimits<std::string>(
      "time_axis:integrator", {{"NEWMARK", "NEWMARK"}, {"SYMPLECTIC", "SYMPLECTIC"}});
  // intervals
  int vint = inparam::gInparamAdvanced.getWithBounds("verbose:loop_info_interval", 1);
  int sint = inparam::gInparamAdvanced.getWithBounds("verbose:stability_interval", 1);
...
releaseToDomain(SE_Model& sem,
  ...
  if (stageI) {
    GradientQuadrature<numerical::Real>::setGMat(spectrals::gGMatrixGLL, spectrals::gGMatrixGLJ);
  }
}
...
initalizeFFT(const std::string& stageKey) {
  // initialize FFT
  double timeLimit =
      inparam::gInparamAdvanced.getWithBounds("develop:time_limit_for_fftw_planning", 0.);
...
releaseSources(SE_Model& sem, Domain& domain, double dt, TimeScheme& timeScheme) {
  ...
  double t1 = inparam::gInparamSource.getWithBounds("time_axis:record_length", 0.);
  int nStepMax = inparam::gInparamAdvanced.get<int>("develop:max_num_time_steps");
  if (nStepMax <= 0) {
  ...
  if (io::gVerbose != io::VerboseLevel::None) {
    ...
    ss << boxEquals(0, 24, "[t0, t1] used by solver", range(step0 * dt, step1 * dt));
...
setupWavefieldScanning(
  ...
  bool enableScanning = inparam::gInparamNr.get<bool>("wavefield_scanning:enable_scanning");
  if (enableScanning) {
